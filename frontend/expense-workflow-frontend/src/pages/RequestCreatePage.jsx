...
import { toRequestLabel } from "../lib/statusLabel"; // 申請ID表示をREQ-xxx形式に変換するために読み込む

import styles from "./RequestCreatePage.module.css";

export default function RequestCreatePage() {
  // /requests/new のページコンポーネントを定義する（新規作成フォームで保存/提出を行う）
  ...
  const createMutation = useMutation({
    // 「保存（下書き作成）」として使う mutation を定義する
    mutationFn: async () => {
      // DRAFT作成（POST /api/requests）を行う非同期関数を定義する
  ...
  const submitNewMutation = useMutation({
    // 「提出（作成→submit）」として使う mutation を定義する
    mutationFn: async () => {
      // 新規作成後に submit を呼ぶ非同期関数を定義する
      const amountNumber = Number(amount || 0); // 文字列の金額を数値へ変換してAPIへ送る準備をする
      const body = { title, amount: amountNumber, note }; // 作成APIが受け取る形に合わせてリクエストボディを作る
      ...
      } // 作成に成功してもidが無い場合はsubmitできないので例外にする
  ...
  return (
    ...
    <div className={styles.container}>
      {" "}
      {/* 見た目を CSS Modules 側へ寄せるためのコンテナを使う */}
      ...
      >
        ...
        <label className={styles.field}>
          ...
          <textarea
            ...
            rows={4}
          />{" "}
          {/* 備考の入力を受け取り、送信中は操作できないようにする */}
        </label>

        <button type="submit" disabled={isSubmitting || !canSubmit}>
          {" "}
          {/* 保存ボタンとして submit を使い、送信中や件名空では押せないようにする */}
          {isSubmitting ? "処理中..." : "保存"}{" "}
          {/* 送信中は表示を変えて二重送信を防ぐ */}
        </button>
        ...
        >
          {" "}
          {/* 提出ボタンは作成→submitを行うため button でクリック処理にする */}
          {isSubmitting ? "処理中..." : "提出"}{" "}
          {/* 送信中は表示を変えて二重送信を防ぐ */}
        </button>
      </form>
