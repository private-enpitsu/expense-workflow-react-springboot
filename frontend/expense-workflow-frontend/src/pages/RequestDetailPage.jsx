...
export default function RequestDetailPage() {
  // /requests/:id のページコンポーネントを定義する（このページは閲覧に寄せる）
    ...
    <div>
      ...
      {isLoading ? ( // ローディング中の分岐を開始する
        ...
        <div>
          ...
          {/* 詳細データの状態を日本語ラベルに変換して表示する */}
          <p>備考：{data.note}</p> {/* 詳細データの備考を表示する */}
          <p>
            履歴件数：{Array.isArray(data.actions) ? data.actions.length : 0}
          </p>{" "}
          {/* actions配列の件数を表示して「配列が返っている」ことを確認する */}
          {canEditReturned ? ( // RETURNED のときだけ「修正する」を表示する分岐を行う
            <p>
              {" "}
              {/* ボタン（リンク）を置く行を開始する */}
              <Link to={`/requests/${requestId}/edit`}>修正する</Link>{" "}
              {/* 編集ページへ遷移して、保存/再提出の操作は編集側へ寄せる */}
            </p> /* 行をここで閉じる */
          ) : null}{" "}
          {/* RETURNED以外は修正導線を出さない */}
        </div> /* 詳細表示ブロックをここで閉じる */
      ) : (
