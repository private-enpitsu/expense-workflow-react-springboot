...
export default function RequestEditPage() {
  // /requests/:id/edit のページコンポーネントを定義する（effect同期を避けて編集開始で初期化する）
  ...
  const ensureEditDraft = () => {
    // 編集開始時に data から state へ初期値を1回だけコピーする関数を定義する
  ...
  const saveMutation = useMutation({
    // RETURNED の申請を「編集して保存」する mutation を定義する
    mutationFn: async () => {
      // 保存API（PATCH）を呼ぶ非同期関数を定義する
      ...
      const amountTextForSave = isEditStarted
        ? amount
        : String(data?.amount ?? 0); // 編集開始後は state、未開始なら data を保存値として扱う
      ...
      const payload = {
        ...
        amount: safeAmount,
        note: noteForSave,
      }; // UpdateRequestRequest 相当のJSONを組み立てる
  ...
  const resubmitMutation = useMutation({
    // 再提出（submit）を実行する mutation を定義する
    mutationFn: async () => {
      // submit APIを呼ぶ非同期関数を定義する
          ...
          {!canEditReturned ? ( // RETURNED以外のときの分岐を開始する
            <div>
              ...
              <p>
                {" "}
                {/* 戻るリンク行を開始する */}
                <Link to={`/requests/${requestId}`}>詳細に戻る</Link>{" "}
                {/* 詳細へ戻して閲覧に寄せる */}
              </p>{" "}
              {/* 戻るリンク行を閉じる */}
            </div> /* 編集不可の案内ブロックを閉じる */
            ...
            <div>
              ...
              <div>
                ...
                />{" "}
                {/* 未編集はdata表示、編集開始でstateへコピーしてからstateを更新する */}
              </div>{" "}
              {/* フォーム行を閉じる */}
              <div>
                {" "}
